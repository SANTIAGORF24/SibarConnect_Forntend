...
import { MediaPicker, AudioRecorder, StickerPicker } from '@/ui/chat';
          ...
          {loading ? (
            ...
            chats.map((chat) => (
              <div
                ...
                className={`flex items-center p-4 cursor-pointer border-b border-gray-100 transition-all duration-200 relative group ${selectedChat?.id === chat.id
                  ? 'shadow-sm'
                  : 'hover:bg-gray-50'
                  }`}
                style={{
                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-3">
                      ...
                      {selectedChat.priority && (
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${selectedChat.priority === 'high' ? 'bg-red-100 text-red-800' :
                    ...
                    return (
                      <div key={message.id}>
                        ...
                        <div className={`flex mb-2 ${message.direction === 'outgoing' ? 'justify-end' : 'justify-start'}`}>
                          <div
                            className={`max-w-xs lg:max-w-md px-3 py-2 rounded-lg shadow-sm ${message.direction === 'outgoing'
                              ? 'bg-[var(--primary)] text-white rounded-br-md'
                              : 'bg-white text-gray-900 border rounded-bl-md'
                              }`}
                          >
                            {/* Solo mostrar el contenido de texto si es un mensaje de texto puro */}
                            {message.content && message.message_type === 'text' && (
                              <p className="text-sm whitespace-pre-wrap">{message.content}</p>
                            )}
                            {renderMediaAttachment(message)}
                            <div className={`flex items-center justify-end mt-1 space-x-1 text-xs ${message.direction === 'outgoing' ? 'text-green-100' : 'text-gray-500'
                              }`}>
                    ...
                    <div className="flex items-start space-x-2 mb-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="mt-3 flex items-center space-x-2">
                          ...
                          <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${lastSummary.interested
                            ? 'bg-green-100 text-green-800'
